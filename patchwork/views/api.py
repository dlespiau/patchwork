...
class SeriesFilter(django_filters.FilterSet):
    ...
    def test_state_filter(queryset, test_state):
        state_choices = {strchoice:intchoice for intchoice,strchoice in TestState.STATE_CHOICES}
        try:
            test_state = state_choices[test_state]
            queryset = queryset.filter(last_revision__test_state=test_state)
        except KeyError:
            if test_state == 'null':
                queryset = queryset.filter(last_revision__test_state__isnull=True)
        return queryset
